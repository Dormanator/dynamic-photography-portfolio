  ...
  async.waterfall(
    [
        ...
        crypto.randomBytes(20, (err, buf) => {
          ...
          if (err) {
            req.flash(
              "error",
              "Sorry, there was trouble creating the temporary link"
            );
        ...
        User.findOne({ email: req.body.email.toLowerCase() }, (err, user) => {
          ...
          user.save(err => {
            if (err) {
              req.flash(
                "error",
                "Sorry, there was trouble creating the temporary token"
              );
              return res.redirect("/forgot");
            }
            done(err, token, user);
      ...
      (token, user, done) => {
        ...
        smtpTransport.sendMail(mailOptions, err => {
          if (err) {
            req.flash(
              "error",
              "Sorry, there was trouble sending the temporary link"
            );
            return res.redirect("/forgot");
          }
          req.flash(
    ...
    err => {
      if (err) {
        req.flash(
          "error",
          "Opps, something went wrong sending your password reset link"
        );
        return next(err);
      }
      res.redirect("/forgot");
            ...
            if (req.body.password === req.body.confirm) {
              user.setPassword(req.body.password, err => {
                if (err) {
                  req.flash(
                    "error",
                    "Sorry, there was trouble setting your new password"
                  );
                  return res.redirect("back");
                }
                ...
                user.save(err => {
                  req.logIn(user, err => {
                    if (err) {
                      req.flash(
                        "error",
                        "Sorry, there was trouble saving your new password"
                      );
                      return res.redirect("back");
                    }
                    done(err, user);
      ...
      (user, done) => {
        ...
        smtpTransport.sendMail(mailOptions, err => {
          if (err) {
            req.flash(
              "error",
              "Sorry, there was trouble sending the confirmation email"
            );
            return res.redirect("back");
          }
