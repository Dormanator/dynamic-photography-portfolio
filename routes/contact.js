...
router.post('/send', (req, res) => {
    ...
    // Verify URL
    const verifyURL = `https://www.google.com/recaptcha/api/siteverify?secret=${secretKey}&response=${captcha}&remoteip=${req
    ...
    request.get(verifyURL, (err, response, body) => {
        ...
        smtpTransport.sendMail(mailOptions, function (err, info) {
            if (err) {
                console.log(err)
                req.flash('error', 'Opps. Something went wrong. Please try again later!');
                res.redirect('/contact');
            } else {
                req.flash('success', 'Thank you for your interest. I will contact you shortly.');
                res.redirect('/photos');
            }
        });
    });
